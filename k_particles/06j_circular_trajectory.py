...
OUTDIR = "/Users/faruk/Documents/temp-k_particles/06j_circular_trajectory"
DIMS = [128, 128]

CENTER = (DIMS[0]-1)/2, (DIMS[1]-1)/2  # Center coordinates of the circle
RADIUS = 8  # Radius of the circle
NUM_POINTS = 300  # Number of points to generate
...
for i in range(NUM_POINTS):
    ...
    for j in range(5):
        if j % 2 == 0:
            coords = generate_points_on_circle(CENTER, RADIUS+j*12, 0, 2*np.pi, NUM_POINTS)
        else:
            coords = generate_points_on_circle(CENTER, RADIUS+j*12, 2*np.pi, 0, NUM_POINTS)
